-- My Classes — cloud sync backend (Supabase)
-- Run this once in your Supabase project's SQL Editor (see SYNC-SETUP.md).
-- It creates one private row per signed-in user to hold that user's data blob,
-- locks it down with row-level security, and turns on live (realtime) updates.

create table if not exists public.user_data (
  user_id    uuid primary key references auth.users(id) on delete cascade,
  data       jsonb not null,
  updated_at timestamptz not null default now()
);

alter table public.user_data enable row level security;

-- Each user can only read/write their own row.
drop policy if exists "own row - select" on public.user_data;
create policy "own row - select" on public.user_data
  for select using (auth.uid() = user_id);

drop policy if exists "own row - insert" on public.user_data;
create policy "own row - insert" on public.user_data
  for insert with check (auth.uid() = user_id);

drop policy if exists "own row - update" on public.user_data;
create policy "own row - update" on public.user_data
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

-- Live updates: let the app hear changes made on your other device in realtime.
alter publication supabase_realtime add table public.user_data;
